...
export const load: PageServerLoad = async ({ params, platform, locals, url }) => {
  ...
  try {
    ...
    const skipHtml =
      ...
      page.view === 'slides' ||
      page.view === 'dashboard';
    const docCacheKey = `page/${page.id}/${page.updated}`;
    const html = skipHtml ? '' : await renderMarkdown(content, { cacheKey: docCacheKey });
    ...
    const seoHtml = skipHtml ? await renderMarkdown(content, { cacheKey: docCacheKey }) : html;
